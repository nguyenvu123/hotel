...
class MonsterInsights_FB_Instant_Articles {
	...
	public function init() {
		...
		if ( ! defined( 'IA_PLUGIN_VERSION'  ) || version_compare( IA_PLUGIN_VERSION, '3.3.5', '<' ) ) {
			...
			if ( ! isset( $_GET['plugin'] ) || isset( $_GET['plugin'] ) && 'fb-instant-articles' !== $_GET['plugin'] ) {
				// AMP Plugin required
	...
	public function requires_ia_plugin() {
		...
		if ( current_user_can( 'install_plugins' ) ) {
			$url = wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=fb-instant-articles' ), 'install-plugin_fb-instant-articles' );
		}
		$text = __( 'Please install the Instant Articles for WP plugin by Automattic version 3.3.5 or newer.', 'monsterinsights-facebook-instant-articles' );
		?>
		<div class="error">
			<p>
				<?php
	...
	public function requires_monsterinsights_version() {
		?>
		<div class="error">
			<p><?php esc_html_e( 'Please install or update MonsterInsights Pro with version 7.4 or newer to use the MonsterInsights Facebook Instant Articles addon', 'monsterinsights-facebook-instant-articles' ); ?></p>
		</div>
		<?php
	}
