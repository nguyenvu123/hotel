...
jQuery(document).ready(function () {
	...
	// Verify license key with Ajax.
	jQuery( document ).on( 'click', '#monsterinsights-verify-submit', function ( event ) {

		event.preventDefault();
		event.stopPropagation();

		verify_license( jQuery(this) );
	} );

	// Deactivate license key with Ajax.
	jQuery( document ).on( 'click', '#monsterinsights-deactivate-submit', function ( event ) {

		event.preventDefault();
		event.stopPropagation();

		deactivate_license( jQuery(this) );
	} );

	// Refresh license key with Ajax.
	jQuery( document ).on( 'click', '#monsterinsights-refresh-submit', function ( event ) {
	...
	function validate_license() {
		...
		var data = {
			action: 'monsterinsights_validate_license',
			nonce: monsterinsights_admin.admin_nonce
		};

		// Show spinner alert similar to redirect.
		swal( {
			type: 'info',
			title: monsterinsights_admin.verify_license_title,
			text: monsterinsights_admin.verify_license_text,
			allowOutsideClick: false,
			allowEscapeKey: false,
			allowEnterKey: false,
			onOpen: function () {
				...
				jQuery.post( ajaxurl, data ).done(
					function ( response ) {
						...
						if ( response.success ) {
							swal( {
								type: 'success',
								title: monsterinsights_admin.verify_license_success_title,
								text: response.data.message,
							...
							swal( {
								type: 'error',
								title: monsterinsights_admin.verify_license_error_title,
	...
	function verify_license( button ) {

		var form = button.closest( 'form' ),
			data = {
				action: 'monsterinsights_verify_license',
				nonce: monsterinsights_admin.admin_nonce,
				license: form.find( '#monsterinsights-settings-key' ).val(),
			};
	...
	function deactivate_license( button ) {

		var form = button.closest( 'form' ),
			data = {
				action: 'monsterinsights_deactivate_license',
				nonce: monsterinsights_admin.admin_nonce,
				license: form.find( '#monsterinsights-settings-key' ).val()
			};
		...
		swal( {
			type: 'info',
			title: monsterinsights_admin.deactivate_license_title,
			text: monsterinsights_admin.deactivate_license_text,
			allowOutsideClick: false,
			allowEscapeKey: false,
			allowEnterKey: false,
			onOpen: function () {
				...
				jQuery.post( ajaxurl, data ).done(
					function ( response ) {
						...
						if ( response.success ) {
							swal( {
								type: 'success',
								title: monsterinsights_admin.deactivate_license_success_title,
								text: response.data.message,
								confirmButtonText: monsterinsights_admin.ok_text,
								allowOutsideClick: false,
								allowEscapeKey: false,
